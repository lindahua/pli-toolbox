function [P, mu, vars, res] = pli_pca(X, k, varargin)
%PLI_PCA Principal component analysis
%
%   [P, mu, vars, res] = PLI_PCA(X, k, ...);
%
%       Performs principal component analysis on a set of data given 
%       as columns of X. It solves the first k principal components.
%
%   [P, mu, vars, res] = PLI_PCA(X, p, ...);
%
...
if k >= 1    
    if k ~= fix(k)
        error('pli_pca:invalidarg', 'k should be a positive integer.');
    end
    if k >= min(d, n)
        error('pli_pca:invalidarg', ...
