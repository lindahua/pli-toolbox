function s = pli_kmpp_seed_c(C, K)
%PLI_KMPP_SEED_C Choose K seeds using K-means++ with pre-computed costs
%
%   s = PLI_KMPP_SEED_C(X, K);
%       
...
if ~(ismatrix(C) && isreal(C) && size(C,1) == size(C, 2))
    error('pli_kmpp_seed_c:invalidarg', 'C should be a real square matrix.');
end
n = size(C, 1);

if ~(isscalar(K) && isreal(K) && K >= 1 && K < n)
    error('pli_kmpp_seed_c:invalidarg', ...
